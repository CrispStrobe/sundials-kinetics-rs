pub mod cantera;
pub mod evaluator;
pub mod jacobian_bridge;
pub mod stiff_solver;
...
/// Arrhenius parameters: k(T) = A * T^n * exp(-Ea / (R*T))
...
pub type Arrhenius = ArrheniusParams;

impl ArrheniusParams {
    pub fn new(a: f64, n: f64, ea: f64) -> Self {
        Self { a, n, ea }
    }

    pub fn rate_constant(&self, temperature: f64) -> f64 {
        const R: f64 = 8.314462;
        self.a * temperature.powf(self.n) * (-self.ea / (R * temperature)).exp()
    }
}
...
pub enum RateLaw {
    ...
    ArrheniusLaw(ArrheniusParams),
    /// Pressure-dependent (falloff) rate law
    PressureDependent { arrhenius: ArrheniusParams, pressure: PressureDependence },
...
impl std::fmt::Debug for RateLaw {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            RateLaw::ArrheniusLaw(a) => write!(f, "Arrhenius(A={}, n={}, Ea={})", a.a, a.n, a.ea),
            RateLaw::PressureDependent { arrhenius, .. } => write!(f, "PressureDependent(A={})", arrhenius.a),
...
impl Clone for RateLaw {
    fn clone(&self) -> Self {
        match self {
            ...
            RateLaw::ArrheniusLaw(a) => RateLaw::ArrheniusLaw(*a),
            RateLaw::PressureDependent { arrhenius, pressure } => RateLaw::PressureDependent { arrhenius: *arrhenius, pressure: pressure.clone() },
...
/// Troe falloff parameters for pressure-dependent reactions.
#[derive(Debug, Clone, Copy)]
...
/// Third-body efficiency for a species in a pressure-dependent reaction.
#[derive(Debug, Clone)]
...
/// Pressure dependence specification for falloff reactions.
#[derive(Debug, Clone)]
pub enum PressureDependence {
    Lindemann {
        high_pressure: ArrheniusParams,
        low_pressure: ArrheniusParams,
...
impl Reaction {
    ...
    pub fn with_arrhenius(a: f64, n: f64, ea: f64) -> Self {
        Self {
            reactants: Vec::new(),
            products: Vec::new(),
            rate_law: RateLaw::ArrheniusLaw(ArrheniusParams::new(a, n, ea)),
        }
    }
...
pub struct ReactionSystem {
    ...
    /// System temperature in Kelvin (for Arrhenius rate evaluation).
    pub temperature: f64,
}
